/**
 * @file main.c
 * @brief Entry point for Nexus ADB Watchdog Version 2.1.
 */
...
static void main_print_usage(const char *argv0)
{
    fprintf(stderr,
            ...
            "Keeps Android TCP ADB (port 5555) available. ESTABLISHED=0 is OK.\n"
            "\n"
            "Usage: %s [-c conf] [-f]\n"
            "  -c conf   Config path (default: %s)\n"
            "  -f        Foreground\n"
            "  -h        Help\n"
            "\n"
            "Test inject (from PC via adb):\n"
...
int main(int argc, char **argv)
{
    ...
    (void)logger_append_line(WATCHDOG_LOG_PATH,
                             "======= Nexus ADB Watchdog 2.1 started =======");

    (void)watchdog_run_loop(&cfg);

    (void)logger_append_line(WATCHDOG_LOG_PATH,
                             "======= Nexus ADB Watchdog 2.1 stopped =======");
