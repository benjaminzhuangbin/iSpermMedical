/**
 * @file logger.c
 * @brief Append-only logging with rotation (Version 2.1).
 */
                         ...
                         int rotate_mb)
{
    ...
    written = fprintf(fp,
                      "\n%s\n"
                      "ADBD=%s PORT5555=%s ESTABLISHED=%d CLIENT=%s\n"
                      "LAST_ACTION=%s RECOVERY_ENABLED=%d RESTART_COUNT=%d COOLDOWN=%ld\n",
                      status->time_str,
                      status->adbd,
                      status->port5555,
                      status->established,
