/**
 * @file config.c
 * @brief Configuration parser for Nexus ADB Watchdog Version 2.1.
 */
...
static void config_parse_line(char *line, watchdog_config_t *cfg)
{
    ...
    *eq = '\0';
    ...
    if (strcmp(key, "interval") == 0 || strcmp(key, "CHECK_INTERVAL") == 0) {
        ...
        cfg->log_enable = (strtol(val, NULL, 10) != 0L) ? 1 : 0;
        ...
        n = strtol(val, NULL, 10);
        if (n >= 0L && n < 86400L) {
            cfg->log_heartbeat_sec = (int)n;
        }
    }
    /* no_client_timeout intentionally ignored in 2.1 (was unsafe in 2.0). */
}
