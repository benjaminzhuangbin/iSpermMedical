/**
 * @file config.c
 * @brief Configuration parser for Nexus ADB Watchdog Version 2.0.
 */
...
void config_set_defaults(watchdog_config_t *cfg)
{
    ...
    cfg->no_client_timeout = WATCHDOG_DEFAULT_NO_CLIENT_TIMEOUT;
...
static void config_parse_line(char *line, watchdog_config_t *cfg)
{
    ...
    *eq = '\0';
    ...
    if (strcmp(key, "interval") == 0 || strcmp(key, "CHECK_INTERVAL") == 0) {
        ...
        n = strtol(val, NULL, 10);
        cfg->recovery_enable = (n != 0L) ? 1 : 0;
    } else if (strcmp(key, "no_client_timeout") == 0) {
        n = strtol(val, NULL, 10);
        if (n > 0L && n < 86400L) {
            cfg->no_client_timeout = (int)n;
        }
    } else if (strcmp(key, "max_restart") == 0 ||
               strcmp(key, "MAX_RESTART") == 0) {
        n = strtol(val, NULL, 10);
        if (n > 0L && n < 100L) {
            cfg->max_restart = (int)n;
        }
    } else if (strcmp(key, "restart_window_sec") == 0) {
        n = strtol(val, NULL, 10);
        if (n > 0L && n < 86400L) {
            cfg->restart_window_sec = (int)n;
        }
    } else if (strcmp(key, "adbd_sleep_sec") == 0) {
        n = strtol(val, NULL, 10);
        if (n > 0L && n < 60L) {
            cfg->adbd_sleep_sec = (int)n;
        }
    }
}
