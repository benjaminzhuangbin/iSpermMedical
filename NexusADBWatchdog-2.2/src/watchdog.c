/**
 * @file watchdog.c
 * @brief Monitor loop + recovery orchestration (Version 2.2).
 */
...
int watchdog_run_once(const watchdog_config_t *cfg, watchdog_status_t *status)
{
    ...
    /* Socket fault heuristic (NOT ESTABLISHED=0). */
    socket_fault = 0;
    ...
    health_classify(adbd_ok, port_ok, prop_ok, tcp.established, tcp.close_wait,
                    socket_fault,
    ...
    /* Policy vs temporary cooldown (never permanent disable). */
