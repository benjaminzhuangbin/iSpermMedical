/**
 * @file recovery.c
 * @brief Auto-recovery for Nexus ADB Watchdog 2.2.
 */
...
void recovery_init(recovery_state_t *st, const watchdog_config_t *cfg)
{
    ...
    st->enabled_runtime = (cfg != NULL && cfg->recovery_enable) ? 1 : 0;
...
long recovery_cooldown_remaining(const recovery_state_t *st)
{
    ...
    if (st == NULL || !st->in_cooldown || st->cooldown_until == 0) {
        return 0;
    }
    now = time(NULL);
...
static int recovery_tick_cooldown(recovery_state_t *st, const watchdog_config_t *cfg)
{
    ...
    recovery_log_block("RECOVERY RESUMED",
                       "cooldown finished; restart_count cleared",
                       "new recovery window");
    return 1;
}
...
static int recovery_rate_allow(recovery_state_t *st, const watchdog_config_t *cfg)
{
    ...
    if (!cfg->recovery_enable || !st->enabled_runtime || st->in_cooldown) {
        return 0;
    }
    ...
    return (st->restart_count < max_r) ? 1 : 0;
}
                      ...
                      int socket_fault)
{
    ...
    st->enabled_runtime = 1;

    /* Healthy service (client optional): adbd + listen + prop + no socket fault */
    if (adbd_ok && port_ok && prop_ok && !socket_fault) {
    ...
    if (!adbd_ok) {
        ...
        recovery_log_block("ADBD DOWN", "FAIL_REASON=ADBD_NOT_RUNNING", "START_ADBD");
        (void)recovery_set_tcp_port();
    ...
    if (!port_ok) {
        ...
        recovery_log_block("PORT 5555 DOWN", "FAIL_REASON=PORT_NOT_LISTENING", "RESTART_ADBD");
    ...
    if (!prop_ok) {
        ...
        recovery_log_block("TCP PORT PROP WRONG", "FAIL_REASON=TCP_PORT_PROP", "FIX_TCP_PORT");
        (void)recovery_set_tcp_port();
        (void)recovery_restart_adbd(cfg->adbd_sleep_sec);
    ...
    /* CASE C: sustained CLOSE_WAIT socket fault (Android-side heuristic) */
    if (socket_fault) {
        if (!recovery_rate_allow(st, cfg)) {
            recovery_enter_cooldown(st, cfg);
            return 0;
        }
        recovery_log_block("TCP FAULT", "FAIL_REASON=TCP_FAULT", "FIX_TCP_FAULT / RESTART_ADBD");
        (void)recovery_restart_adbd(cfg->adbd_sleep_sec);
