/**
 * @file status.c
 * @brief Status snapshot helpers and status-file writer.
 */
...
void status_set_time_now(watchdog_status_t *status)
{
    ...
    /* localtime_r is available on Android Bionic / glibc / musl. */
...
int status_write_file(const char *path, const watchdog_status_t *status)
{
    ...
    /*
     * Write via temporary file then rename for atomic replace on the same FS.
     * Fallback: direct overwrite if rename fails.
     */
    {
        char tmp_path[256];
        int  n;
