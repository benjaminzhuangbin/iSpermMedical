/**
 * @file process.c
 * @brief Process scanning via /proc and PID file helpers.
 */
...
static int process_matches(const char *exe_path, const char *exe, const char *cmdline)
{
    ...
    if (exe != NULL && exe[0] != '\0') {
        ...
        /* Android may report "(deleted)" suffix after in-place replace. */
        if (strncmp(exe, exe_path, strlen(exe_path)) == 0) {
            return 1;
        }
        if (strstr(exe, base) != NULL) {
            /* Prefer exact basename match at end of path component. */
    ...
    if (cmdline != NULL && cmdline[0] != '\0') {
        ...
        /* Match first argv token equal to basename (e.g. "adbd"). */
...
int process_is_running(const char *exe_path)
{
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        if (process_read_exe(ent->d_name, exe, sizeof(exe)) != 0) {
            /* Unprivileged or kernel thread; try cmdline only. */
            errors++;
        }
    ...
    /* If /proc was empty of readable PIDs, report error. */
    (void)errors;
    return 0;
}
