/**
 * @file main.c
 * @brief Entry point for Nexus ADB Watchdog daemon (Version 1).
 *
...
int main(int argc, char **argv)
{
    ...
    /*
     * Version 1: stay in foreground of the launching shell job.
     * Operators are expected to use: nohup /data/local/watchdog/watchdog &
     * The -f flag is reserved for explicit console attachment / debugging.
     */
    (void)foreground;
    ...
    (void)logger_append_line(WATCHDOG_LOG_PATH,
                             "======= Nexus ADB Watchdog started =======");

    (void)watchdog_run_loop(&cfg);

    (void)logger_append_line(WATCHDOG_LOG_PATH,
                             "======= Nexus ADB Watchdog stopped =======");
