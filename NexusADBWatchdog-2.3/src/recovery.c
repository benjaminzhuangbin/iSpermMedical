/**
 * @file recovery.c
 * @brief Auto-recovery for Nexus ADB Watchdog 2.3.
 *
                                  ...
                                  int abnormal_tcp)
{
    ...
    /* Any sustained FIN_WAIT / LAST_ACK / CLOSING on :5555 is abnormal. */
    if (abnormal_tcp > 0) {
        if (st->session_stale_since == 0) {
            st->session_stale_since = now;
            return 0;
        }
        if ((now - st->session_stale_since) >= (time_t)hold) {
            return 1;
        }
        return 0;
    }

    st->session_stale_since = 0;
    return 0;
}
...
void recovery_note_healthy(recovery_state_t *st)
{
    ...
    if (st->pending_verify) {
        ...
        recovery_log_block("RECOVERY SUCCESS",
                           "ADBD=YES PORT5555=YES TCP/ADB health OK",
                           "FAIL_REASON=NONE");
        st->pending_verify = 0;
...
static int recovery_rate_allow(recovery_state_t *st, const watchdog_config_t *cfg)
{
    ...
    if (st->restart_count >= max_r) {
        ...
        /* Show remaining window time in COOLDOWN= without entering post-action cooldown. */
                      ...
                      int protocol_fault)
{
    ...
    service_ok = (adbd_ok && port_ok && prop_ok &&
                  !socket_fault && !session_stale && !protocol_fault);
    ...
    if (!adbd_ok) {
        ...
        recovery_log_block("ADB FAULT", "FAIL_REASON=ADBD_NOT_RUNNING", "START_ADBD");
        (void)recovery_set_tcp_port();
    ...
    if (!port_ok) {
        ...
        recovery_log_block("ADB FAULT", "FAIL_REASON=PORT_NOT_LISTENING", "RESTART_ADBD");
    ...
    if (!prop_ok) {
        ...
        recovery_log_block("ADB FAULT", "FAIL_REASON=TCP_PORT_PROP", "FIX_TCP_PORT");
        (void)recovery_set_tcp_port();
        (void)recovery_restart_adbd(cfg->adbd_sleep_sec);
    ...
    /* CASE D: ADB protocol/session health (PC offline-class on Android side) */
    if (protocol_fault) {
        if (!recovery_rate_allow(st, cfg)) {
            return 0;
        }
        recovery_log_block("ADB FAULT", "FAIL_REASON=ADB_PROTOCOL_FAULT",
                           "FIX_ADB_PROTOCOL / RESTART_ADBD");
        (void)recovery_restart_adbd(cfg->adbd_sleep_sec);
        (void)adb_proto_inject_fault_clear();
        util_strlcpy(st->last_action, RECOVERY_ACTION_FIX_ADB_PROTOCOL,
                     sizeof(st->last_action));
        recovery_log_block("FIX_ADB_PROTOCOL", "adbd restarted; inject cleared", "");
        recovery_note_attempt(st, cfg);
        return 1;
    }

    /* CASE C: session stale (FIN_WAIT / LAST_ACK heuristic) */
    if (session_stale) {
        if (!recovery_rate_allow(st, cfg)) {
            return 0;
        }
        recovery_log_block("ADB FAULT", "FAIL_REASON=SESSION_STALE",
                           "FIX_TCP_FAULT / RESTART_ADBD");
        (void)recovery_restart_adbd(cfg->adbd_sleep_sec);
        util_strlcpy(st->last_action, RECOVERY_ACTION_FIX_TCP_FAULT, sizeof(st->last_action));
        recovery_log_block("FIX_TCP_FAULT", "session stale; adbd restarted", "");
        st->session_stale_since = 0;
        recovery_note_attempt(st, cfg);
        return 1;
    }
    ...
    if (socket_fault) {
        ...
        recovery_log_block("ADB FAULT", "FAIL_REASON=TCP_FAULT", "FIX_TCP_FAULT / RESTART_ADBD");
        (void)recovery_restart_adbd(cfg->adbd_sleep_sec);
        util_strlcpy(st->last_action, RECOVERY_ACTION_FIX_TCP_FAULT, sizeof(st->last_action));
        recovery_log_block("FIX_TCP_FAULT", "adbd restarted", "");
        st->socket_fault_since = 0;
        recovery_note_attempt(st, cfg);
        return 1;
    }
