/**
 * @file main.c
 * @brief Entry for Nexus ADB Watchdog Version 2.3.
 */
...
static void main_print_usage(const char *argv0)
{
    fprintf(stderr,
            ...
            "Layer 4: localhost ADB CNXN protocol health check.\n"
            "\n"
            "Usage: %s [-c conf] [-f]\n"
            ...
            "  echo ADB_PROTOCOL_FAULT > /data/local/watchdog/watchdog.inject\n",
...
int main(int argc, char **argv)
{
    ...
    (void)logger_append_line(WATCHDOG_LOG_PATH,
                             "======= Nexus ADB Watchdog 2.3 started =======");
    (void)watchdog_run_loop(&cfg);
    (void)logger_append_line(WATCHDOG_LOG_PATH,
                             "======= Nexus ADB Watchdog 2.3 stopped =======");
