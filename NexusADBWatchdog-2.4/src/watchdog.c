                               ...
                               int heartbeat_sec)
{
    ...
    if (strcmp(status->fail_reason, g_prev_reason) != 0) return 1;
...
static void watchdog_remember_snap(const watchdog_status_t *status)
{
    ...
    util_strlcpy(g_prev_reason, status->fail_reason, sizeof(g_prev_reason));
...
int watchdog_run_once(const watchdog_config_t *cfg, watchdog_status_t *status)
{
    ...
    /* CLOSE_WAIT heuristic only (NOT ESTABLISHED=0, NOT CNXN probe). */
    socket_fault = 0;
    ...
    health_classify(adbd_ok, port_ok, prop_ok, tcp.established, socket_fault,
                    ...
                    status->client_state, (unsigned int)sizeof(status->client_state),
